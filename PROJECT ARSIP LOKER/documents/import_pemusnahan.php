    ...
    try {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            try {
                ...
                // Cek duplikasi dokumen di lemari pemusnahan (status deleted)
                $existing = $db->fetch(
                    "SELECT id FROM documents WHERE (nik = ? OR passport_number = ?) AND status = 'deleted'",
                    [$nik, $passport_number]
                );
                ...
                if ($existing) {
                    $duplicate_rows[] = [
                        'row' => $row_number,
                        'reason' => 'Dokumen sudah ada di lemari pemusnahan (NIK atau Passport duplikat)',
                        'data' => $full_name
                    ];
                    $duplicate_count++;
                    continue;
                ...
                // Validate locker optional
                $locker = null;
                if ($month_number) {
                    $locker = $db->fetch("SELECT id FROM lockers WHERE code = ?", [$month_number]);
                    if (!$locker) {
                        $failed_rows[] = [
                            'row' => $row_number,
                            'reason' => "Kode Lemari ('$month_number') tidak ditemukan.",
                            'data' => $full_name
                        ];
                        $failed_count++;
                        continue;
                ...
                $params = [
                    ...
                    $nik,
                    $passport_number,
                    null, // birth_date tidak dipakai di import pemusnahan ini
                    $month_number,
        ...
        if ($imported_count > 0) {
            ...
            if ($duplicate_count > 0) {
                $success_message .= " <strong>Tidak dapat diimport (duplikat): $duplicate_count dokumen</strong> - dokumen sudah ada di lemari pemusnahan.";
            }
            if ($failed_count > 0) {
            ...
            if ($duplicate_count > 0) {
                $error_message .= " <strong>Tidak dapat diimport (duplikat): $duplicate_count dokumen</strong> - dokumen sudah ada di lemari pemusnahan.";
            }
            if ($failed_count > 0) {
...
if ($duplicate_count > 0 && !empty($duplicate_rows)) {
